...
Установить Claude Code CLI: https://claude.ai/code
...
Создайте `.env` в папке приложения для передачи переменных в claude:
```env
ANTHROPIC_API_KEY=...
```
...
**`'claude' is not recognized`** — установите Claude Code CLI и добавьте его в PATH.

**Приложение зависло в MANUAL режиме** — это нормально, если все разрешения сняты. Нажмите **■ СТОП**.

**Ошибка кодировки в run.bat** — файл должен быть сохранён в ASCII (без кириллицы). Используйте оригинальный `run.bat` из репозитория.
...
Create `.env` in the app folder to pass variables to claude:
```env
ANTHROPIC_API_KEY=...
```
...
**`'claude' is not recognized`** — install Claude Code CLI and add it to PATH.

**App hangs in MANUAL mode** — expected if all permissions are unchecked. Press **■ STOP**.

**Encoding error in run.bat** — file must be saved as ASCII (no Cyrillic). Use the original `run.bat` from the repository.
